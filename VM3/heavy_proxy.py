...
@app.route('/init')
def cloud_init():
    [img, logs] = client.images.build(path='/home/comp598-user/', rm = True, dockerfile='/home/comp598-user/Dockerfile.heavy')
    node_name = 'default_1'
    port = '17001'
    running = False
    client.containers.create(image= img,
                          detach=True,
                          name='Heavy_' + node_name,
                          ports = {'5030/tcp':'17001'},
                          cpu_percent= 80,
                          mem_limit= '500m')
...
def cloud_register(node_name, port):
    if len(node_list) == 10:
        return jsonify({'response': 'register node files',
            'reason': 'the Light pod can take maximum 10 nodes'})
    ...
    client.containers.create(image= img,
                          detach=True,
                          name=str('Heavy_'+node_name),
                          ports = {'5030/tcp':str(port)},
                          cpu_percent= 80,
                          mem_limit= '500m')
    node_list.append({'port':port, 'name':node_name, 'running': False})
    return jsonify({'response': 'success',
...
def cloud_rm(node_name):
    ...
    for container in client.containers.list(all=True):
        if container.name == str('Heavy_'+node_name):
...
def launch_node(container_name, port_number):
    [img, logs] = client.images.build(path='/home/comp598-user/', rm = True, dockerfile='/home/comp598-user/Dockerfile.heavy')
    for container in client.containers.list(all=True):
        if container.name == str('Heavy_'+container_name):
            container.remove(v=True, force= True)
            break
    client.containers.run(image= img,
                          detach=True,
                          name=str('Heavy_'+container_name),
                          command=['python3', 'h_job.py',str('Heavy_'+container_name) ],
                          ports = {'5030/tcp': port_number},
                          cpu_percent= 80,
                          mem_limit= '500m')
...
def monitor_resources():
    ...
    for node in node_list:
        if node['running']:
            names.append('Heavy_' + node['name'])

    for container in client.containers.list(all=True):
        if container.name in names:
            heavy_containers.append(container)
    ...
    if len(heavy_containers) != 0:
        cpu_usage = cpu_usage/len(heavy_containers)
        mem_usage = mem_usage/len(heavy_containers)
...
def elastic_remove(name):
    ...
    for container in client.containers.list(all=True):
        if container.name == str('Heavy_' + name):
            container.stop()
            container.remove()
            break
